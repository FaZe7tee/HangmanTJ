...
def start_game():
    ...
    def update_display():
        ...
        attempts_label.config(text="Attempts remaining: " + str(attempts_left))
    ...
    attempts_label = tk.Label(game_window, text="Attempts remaining: " + str(attempts_left))
    attempts_label.pack()
...
def save_high_score():
    ...
    if current_user in high_scores:
        if attempts_left > int(high_scores[current_user]):
            high_scores[current_user] = str(attempts_left)
    else:
        high_scores[current_user] = str(attempts_left)
